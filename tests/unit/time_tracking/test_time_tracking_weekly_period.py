...
def test_monday_twelve_utc_closing() -> None:

    c = work_week_monday_twelve_closing_aware(
        date(2024, 1, 6),
        tz_name="UTC",
    )
    assert c == datetime(2024, 1, 15, 12, 0, 0, tzinfo=timezone.utc)


def test_deadline_passed_around_monday_twelve_utc() -> None:
    wd = date(2024, 1, 10)
    before = datetime(2024, 1, 15, 11, 59, 0, tzinfo=timezone.utc)
    after = datetime(2024, 1, 15, 12, 0, 0, tzinfo=timezone.utc)
