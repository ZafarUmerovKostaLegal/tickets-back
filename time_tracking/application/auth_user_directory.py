...
from infrastructure.auth_directory_cache import get_auth_partner_hints, set_auth_partner_hints
from infrastructure.config import get_settings

_log = logging.getLogger(__name__)


def _parse_partner_hints_list(data: list) -> dict[int, dict[str, str | None]]:
...
async def fetch_auth_user_positions_by_id(authorization: str) -> dict[int, str | None]:
    hints = await fetch_auth_user_partner_hints_by_id(authorization)
    return {uid: (h.get("position") if isinstance(h, dict) else None) for uid, h in hints.items()}
...
async def fetch_auth_user_partner_hints_by_id(
    ...
    if not authz:
        return {}
    cached = get_auth_partner_hints(authz)
    ...
    if not isinstance(data, list):
        return {}
    out = _parse_partner_hints_list(data)
    set_auth_partner_hints(authz, out)
    return out
