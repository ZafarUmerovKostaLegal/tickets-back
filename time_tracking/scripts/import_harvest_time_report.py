...
DEFAULT_HARVEST_FILE = TT_ROOT / "newreport.csv"
HARVEST_REPORT_GLOB_PATTERNS = (
    "newreport*.csv",
    "newreport*.xlsx",
    "harvest_time_report*.csv",
    "harvest_time_report*.xlsx",
)
...
def _glob_harvest_reports(directory: Path) -> list[Path]:
    """CSV/XLSX отчёты Harvest в каталоге, новейшие первыми."""
    try:
        matches = [
            p
            for pattern in HARVEST_REPORT_GLOB_PATTERNS
...
def _harvest_file_candidates(preferred: Path) -> list[Path]:
    ...
    if is_default:
        for path in (TT_ROOT / "newreport.csv", TT_ROOT / "newreport.xlsx", preferred):
            if path.is_file():
                add(path)
...
def main() -> int:
    ...
    p.add_argument(
        ...
        help=f"Путь к .csv или .xlsx (по умолчанию: {DEFAULT_HARVEST_FILE.name} в time_tracking/).",
    )
    p.add_argument(
        "--database-url",
        type=str,
    ...
    if harvest_file is None:
        ...
        print(f"Ожидаемые имена: newreport.csv, {HARVEST_CSV_NAME} или {HARVEST_XLSX_NAME}")
