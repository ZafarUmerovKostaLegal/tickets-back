...
class TimeManagerClientTaskOut(BaseModel):
    ...
    flat_fee_amount: Optional[Decimal] = Field(None, alias="flatFeeAmount")
    flat_fee_currency: Optional[str] = Field(None, alias="flatFeeCurrency")
    created_at: datetime
    updated_at: Optional[datetime] = None
...
class TimeManagerClientProjectOut(BaseModel):
    ...
    usage_count: int = 0
    deletable: bool = True
    budget_display_value: Optional[float] = Field(None, alias="budgetDisplayValue")
...
class TimeManagerClientProjectCreateBody(BaseModel):
    ...
    initial_time_tracking_user_billable_hourly_amounts: list[Optional[Decimal]] = Field(
        ...
        description=(
            "Опционально: billable за час по проекту для каждого id из initialTimeTrackingUserAuthIds "
            "в том же порядке и той же длине. Сохраняется с привязкой к проекту. Несовместимо с initialProjectAccessMembers."
        ),
    )
    ...
    def _initial_access_billable_lists(self) -> Self:
        ...
        if members and amts:
            raise ValueError(
                "Задайте либо initialProjectAccessMembers, либо пару initialTimeTrackingUserAuthIds + "
