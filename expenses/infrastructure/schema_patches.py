...
async def apply_expense_rejection_reason(conn: AsyncConnection) -> None:
    await conn.execute(
        text("ALTER TABLE expense_requests ADD COLUMN IF NOT EXISTS rejection_reason TEXT")
    )
    await conn.execute(
        text(
            """
            UPDATE expense_requests AS er
            SET rejection_reason = src.comment
            FROM (
                SELECT DISTINCT ON (expense_request_id)
                    expense_request_id,
                    comment
