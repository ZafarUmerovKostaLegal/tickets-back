...
def get_notifications_upload_dir() -> Path:
    path = safe_media_path(get_settings().media_path, "notifications")
    if path is None:
...
def save_photo(filename: str, content: bytes) -> str:
    ...
    get_notifications_upload_dir()
    ext = Path(filename).suffix if filename else ""
    unique_name = f"{uuid.uuid4().hex}{ext}"
    path = safe_media_path(get_settings().media_path, f"notifications/{unique_name}")
    if path is None:
        raise ValueError("Invalid media path")
